...
public class TikaCtakesResource {
  ...
  public Response forwardCtakes(InputStream is,
    ...
    String tikaOutputFile = shangridocsHome + "/shangridocs-services/src/main/resources/buffer/tikaOutDir/output.txt";
    String duccOutputFile = shangridocsHome + "/shangridocs-services/src/main/resources/buffer/duccOutDir/output.txt";
    ...
    String json = submitDuccJob(jobFile, tikaOutputFile, duccOutputFile);
  ...
  private String submitDuccJob(String jobFile, String tikaOutputFile, String duccOutputFile) {
    String output = "";
    try {
      ...
      try {
        ...
        ProcessBuilder handlerPb = new ProcessBuilder(new String[] { handlerHome+"/lib/run.sh", "-i", duccOutputFile + ".xml", "-o", tikaOutputFile});
        String cmd = "";
        for (String arg : handlerPb.command())
            cmd = cmd + " " + arg;
        LOG.info("Running process: "+cmd);
        process = handlerPb.start();
        //Must refresh io handles
        BufferedReader stdInput2 = new BufferedReader(new InputStreamReader(process.getInputStream()));
        BufferedReader stdError2 = new BufferedReader(new InputStreamReader(process.getErrorStream()));
        // Read any errors from the attempted command
        LOG.info("Std out:");
        String err;
        String out;
        //Non-shortcircuit operator intended
        while (((err = stdError2.readLine()) != null) | ((out = stdInput2.readLine()) != null)) {
      ...
      InputStream inputStream = new FileInputStream(tikaOutputFile);
